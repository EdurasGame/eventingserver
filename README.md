...
Server and clients communicate via so called *Events*. Events have a unique identifier and come with information provided by you. To create an Event, use the `Event(int eventNumber)` constructor of the `Event` class. If you want two Events to be processed differently you must assign them different numbers. To put information into the event use the `putArgument(Object argument)` method. Floats, Doubles, Strings, Integers, Longs and Booleans will be serialized automatically. For any other kind of object you will have to provide your own serialization.

Once an Event arrives at either the client or the server the respective `EventHandler` is called to process your event. Your job is to define an EventHandler for the client and server (not necessarily the same) where you determine depending on the Event (make a switch over the event number) what to do. You can access the information contained in the event with the `getArgument(int i)` method. The arguments are placed in the same order as they had been put on the other side before.
